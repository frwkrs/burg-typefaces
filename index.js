...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, callback) {
    ...
    if (file.fieldname === 'pictures') {
      callback(null, req.files['fontFile'][0].originalname + '-' + pictureCount + path.extname(file.originalname));
      pictureCount++;
    } else {
...
app.post("/upload", upload.fields([{name: 'fontFile', maxCount: 1}, {name: 'styleFiles', maxCount: 12}, {name: 'pictures', maxCount: 5}]), limit({
  max: 5, // 5 requests
  period: 60 * 1000, // per minute (60 seconds)
}), async (req, res) => {
...
async function createCSSFiles(fontsFolder, stylesFolder, cssFile) {
  ...
  for (const fontFile of fontFiles) {
    ...
    // Create the CSS file
    fs.appendFileSync(`${cssFile}/${fontName}.css`, `@font-face {
        font-family: '${fontName}';
    ...
    const styleFiles = fs.readdirSync(stylesFolder).filter(file => file.startsWith(`${fontName}-`));

    // Iterate over each style file
    for (const styleFile of styleFiles) {
        const style = styleFile.split('.')[0];
         fs.appendFileSync(`${cssFile}/${fontName}.css`, `\n@font-face {
            font-family: '${style}';
